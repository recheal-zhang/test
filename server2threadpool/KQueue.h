...
pthread_mutex_t kqueueLock = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t kqueueCond = PTHREAD_COND_INITIALIZER;
//生产者消费者中间变量
std::queue<int> fdQue;
bool kQueueFlags = false;
...
void decrementCount(){
    ...
    pthread_cond_wait(&count_nonzero, &threadLock);
...
void *serverSocket(unsigned int *param){
    ...
    for(;;){
        ...
        pthread_mutex_lock(&kqueueLock);
        pthread_cond_signal(&kqueueCond);
        pthread_mutex_unlock(&kqueueLock);

        std::cout << pthread_self() << std::endl;
        int n = 0;

        if(fd <= 0){
            std::cout << "server socket fd error" << std::endl;
            continue;
        }
...
static void initThreadPool(void){
    ...
    for(int i = 0; i < THREAD_NUM; i++){
        ...
        threadPara[i][7] = i;
        tag = pthread_create(threadId + i, NULL, (void *(*)(void *))(serverSocket), (void *)(threadPara[i]));
        if(tag != 0){
...
class KQueue{
        friend void *serverSocket(unsigned int *param);
    public:
        KQueue(){}
...
void KQueue::handleAccept(int efd, int fd){
    ...
    std::cout << "accept & updateEvents done" << std::endl;
}
    ...
    for(int i = 0; i < n; i++){
        ...
        if(events & EVFILT_READ){
            if(fd == listenFd){
                ...
                pthread_mutex_lock(&kqueueLock);
                pthread_cond_wait(&kqueueCond, &kqueueLock);
                pthread_mutex_unlock(&kqueueLock);
